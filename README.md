...
- `presentmentPrices` - Optional - Whether to include the header to pull
  presentment prices for products. Defaults to `false`. This option allows the
  use of a custom JSON serializer that might be needed to properly handle long
  integer IDs. Defaults to `JSON.stringify()`.
- `stringifyJson` - Optional - The function used to serialize to JSON. The
  function is passed a single argument.
